...
/*Additonal function
 * This function check if the linked list is empty.
...
/*Additonal function
 * This function get the value of the first node in the list.
 ...
 * 
 * Return:
 *   The value stored in the first node (top) of the list, or NULL if the list is empty.
 */
void* top_value(struct list* list){
  if(list->head == NULL){
    return NULL;
  }
  void* top_value = list->head->val;
  return top_value;
}

/*Additonal function
 * This function remove and returns the value of the first node in the list.
 ...
 * 
 */
void* pop_value(struct list* list){
  if (list->head == NULL) {
        return NULL; // List is empty, return NULL
  }
  
  struct node* curr;
  void* return_value =  list->head->val;
  curr = list->head;
  list->head = curr->next;
  free(curr);
  return return_value;
...
/*Additonal function
 * This function retrieves the size of a linked list by counting the number 
 * of nodes in the list.
 *
 * It starts from the head of the list and traverses each node, incrementing 
 * a counter until it reaches the end of the list (where the next pointer 
 * is NULL). The final count represents the total number of elements in the 
 * linked list.
 *
 * Params:
 *   list - the linked list whose size is to be calculated. It may not be NULL.
 *
 * Return:
